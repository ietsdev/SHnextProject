...
export const Agregar = (raza: Raza) => {
  ...
  return fetch('https://localhost:5001/Razas/Agregar', {
    method: 'POST',
    headers:{
...
export async function getRazas() : Promise<Raza[]>{
  ...
  process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = '0';
  return await fetch('https://localhost:5001/Razas/Listar') 
...
export async function deleteRaza(id:number) : Promise<Raza>{
  process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = '0';
  return await fetch('https://localhost:5001/Razas/Eliminar/' + id,
          {
            method: 'DELETE',
            headers:{
              'Content-Type': 'application/json'
